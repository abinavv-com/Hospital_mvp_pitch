...
const { Card, Button, IconButton, Avatar, StatusPill, Badge } = window.MedKonectDesignSystem_4460c8;

function VideoHero({ joined, onJoin }) {
  return (
      ...
      <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", justifyContent: "space-between", padding: 32,
        ...
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 24, flexWrap: "wrap" }}>
          <div className="mk-glass-dark" style={{ padding: 24, borderRadius: "var(--radius-xl)", borderLeft: "4px solid var(--mk-teal-500)", maxWidth: 460, flex: 1 }}>
            ...
            <h2 className="mk-headline-md" style={{ color: "#fff", marginBottom: 12 }}>Patient: Sarah Montgomery</h2>
            <div style={{ display: "flex", gap: 24, marginBottom: 20, flexWrap: "wrap" }}>
              <span style={{ display: "flex", alignItems: "center", gap: 8, color: "rgba(255,255,255,0.85)", fontSize: 14 }}>
                <span className="material-symbols-outlined" style={{ color: "var(--mk-teal-300)", fontSize: 18 }}>schedule</span>10:30 AM (In 4 mins)</span>
              <span style={{ display: "flex", alignItems: "center", gap: 8, color: "rgba(255,255,255,0.85)", fontSize: 14 }}>
...
function Teleconsult({ onNavigate }) {
    ...
    <>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 32 }} className="mk-dashgrid">
        <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
          <VideoHero joined={joined} onJoin={() => setJoined((v) => !v)} />
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
          ...
          <Card variant="ink" style={{ position: "relative", overflow: "hidden" }}>
            ...
            <p style={{ fontSize: 13, lineHeight: "20px", color: "var(--mk-blue-100)" }}>
              Sarah reported increased morning headaches and blurred vision in yesterday's health-kit sync. BP logged at 155/95. Prioritize blood pressure medication review.</p>
          </Card>
        </div>
      </div>
    </>
