...
const { Card, Button, Input, Avatar, StatusPill } = window.MedKonectDesignSystem_4460c8;
    ...
    <Card variant="glass" interactive accent={false} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: 20, borderRadius: "var(--radius-lg)" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 24 }}>
        <div style={{ textAlign: "right" }} className="mk-hide-sm">
          <StatusPill variant={r.variant}>{r.status}</StatusPill>
          <div className="mk-caption" style={{ color: "var(--text-subtle)", marginTop: 4 }}>Ref {r.ref}</div>
        </div>
...
function Records() {
  ...
  return (
    <>
      ...
      <h3 className="mk-headline-md" style={{ marginBottom: 16 }}>Recent Documents</h3>
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        {d.records.map((r, i) => <DocRow key={i} r={r} />)}
      </div>
    </>
  );
}
